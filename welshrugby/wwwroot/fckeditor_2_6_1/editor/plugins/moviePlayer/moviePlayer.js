...
function Ok()
{
	...
	if ( rbFileTypeVal == "single") {
		if ( GetE('txtURL').value.length == 0 )
		{
			...
			alert( oEditor.FCKLang.DlgmoviePlayerAlertUrl ) ;
			return false ;
		}
	}
	...
	if (rbFileTypeVal == "list") {
		if ( GetE('txtPlaylist').value.length == 0 )
		{
			...
			alert( oEditor.FCKLang.DlgmoviePlayerAlertPlaylist ) ;
			return false ;
		}
	}
	...
	if ( GetE('txtWidth').value.length == 0 )
	{
		...
		alert( oEditor.FCKLang.DlgmoviePlayerAlertWidth ) ;
		return false ;
	}
	...
	if ( GetE('txtHeight').value.length == 0 )
	{
		...
		alert( oEditor.FCKLang.DlgmoviePlayerAlertHeight ) ;
		return false ;
	}
...
Media.prototype.getInnerHTML = function (objectId){
	...
	s+= '<div style="border-style: none; height: ' + thisHeight + 'px; width: ' + thisWidth + 'px; overflow: hidden; background-color: rgb(220, 220, 220); background-image: url(' + oEditor.FCKConfig.PluginsPath + 'moviePlayer/moviePlayer.gif); background-repeat:no-repeat; background-position:center;' + cssalign + '">';
	s+= '<script src="' + oEditor.FCKConfig.PluginsPath + 'moviePlayer/swfobject.js" type="text/javascript"></script>\n';
	...
	//s+= '	var s1 = new SWFObject("' + oEditor.FCKConfig.PluginsPath + 'moviePlayer/mediaplayer.swf","' + thisMediaType + '","' + thisWidth + '","' + thisHeight + '","7");\n';
	//s+= '	s1.addVariable("width","' + thisWidth + '");\n';
	//s+= '	s1.addVariable("height","' + thisHeight + '");\n';
	//s+= '	s1.addVariable("autostart","' + this.play + '");\n';

	s+= '<embed\n';
	s+= '	id="player' + randomnumber + '"\n';
	s+=	'	src="/res/mov/mediaPlayer.swf"\n';
	s+=	'	width="' + thisWidth + '"\n';
	s+= '	height="' + thisHeight + '"\n';
	s+= '	allowscriptaccess="always"\n';
	s+= '	allowfullscreen="' + this.fullscreen + '"\n';
	s+= '	autostart=' + this.play + '\n';
	s+= '	flashvars="';
		s+= 'height=' + thisHeight;
		s+= '&width=' + thisWidth;
		s+= '&autostart=' + this.play;
	...
	*/
	s+= '&showdownload=' + this.downloadable;
	s+= '&link=' + this.url;
	s+= '&allowfullscreen=' + this.fullscreen
	s+= '&showdigits=' + this.displayDigits;
	s+= '&shownavigation=' + this.displayNavigation;
	...
	s = '<div>VIDEO|'+thisWidth+'x'+thisHeight+'|'+this.url+'|'+this.play+'|VIDEO</div>\n';
