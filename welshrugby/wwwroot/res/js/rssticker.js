...
//if (window.getComputedStyle($(this.tickerid), "")) //detect if moz-opacity is defined in external CSS for specified class
//this.mozopacityisdefined=(window.getComputedStyle(this.tickerid, "").getPropertyValue("-moz-opacity")==1)? 0 : 1
this.getAjaxcontent()
}
...
this.ajaxobj.open('GET', this.RSS_url, true)
...
rssticker_ajax.prototype.fadetransition = function(fadetype, timerid) {
	if (fadetype=="reset")
		Element.hide(this.tickerid);
	else
		Effect.Appear(this.tickerid);
}